  ...
  pop() {
    ...
    while(currentIdx < lastIdx) {
      ...
      if(leftChildIdx > lastIdx) {
        ...
        if(this.heap[leftChildIdx] > this.heap[currentIdx]) { // 왼쪽 자식 노드가 현재 노드 보다 더 큰 경우
        ...
        } else if(this.heap[leftChildIdx] > this.heap[currentIdx]) { // 왼쪽 자식 노드만 현재 노드 보다 클 경우

          this.swap(currentIdx, leftChildIdx);
          currentIdx = leftChildIdx;

        } else if(this.heap[rightChildIdx] > this.heap[currentIdx]) { // 오른쪽 자식 노드만 현재 노드 보다 클 경우

          this.swap(currentIdx, rightChildIdx);
          currentIdx = rightChildIdx;

        } else { // 자식 노드 2개 모두 현재 노드 보다 작을 경우
